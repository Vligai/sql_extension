...
public class generated {
	...
	public static void main(String[] args)
		...
		arr_str = "(mf_struct[i].cust).equals(r.cust), (mf_struct[i].cust).equals(r.cust) && r.year == 1990, (mf_struct[i].cust).equals(r.cust) && r.year == 1991, (mf_struct[i].cust).equals(r.cust) , null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null";
			...
			for (ii = 0; ii < max; ii++) {
				...
				for (i = 0; i < max; i++) {
					if (mf_struct[i] == null)
						break;
					/* generated code */
					if ((counter2 == 1) && ((mf_struct[i].cust).equals(r.cust) && r.year == 1990)) {
						mfstruct_gen.update_mfstruct(r, i, counter2);
					}
					if ((counter2 == 2) && ((mf_struct[i].cust).equals(r.cust) && r.year == 1991)) {
						mfstruct_gen.update_mfstruct(r, i, counter2);
					}
					/* end generated code */
				}
		...
		while (i < max) {
			...
			if (my_input[5][0].equals("null")) {
				...
				try {
					System.out.println(
							mf_struct[i].cust + " " + ((double) mf_struct[i].sum_1_quant / mf_struct[i].count_1_quant)
									+ " " + ((double) mf_struct[i].sum_2_quant / mf_struct[i].count_2_quant));
				} catch (Exception e) {
				...
				if (true) {
					...
					try {
						System.out.println(mf_struct[i].cust + " "
								+ ((double) mf_struct[i].sum_1_quant / mf_struct[i].count_1_quant) + " "
								+ ((double) mf_struct[i].sum_2_quant / mf_struct[i].count_2_quant));
					} catch (Exception e) {
						System.out.println(e.getMessage());
					}
